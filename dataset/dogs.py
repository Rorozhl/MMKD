...
class DogsSample(VisionDataset):
    """`Stanford Dogs <http://vision.stanford.edu/aditya86/ImageNetDogs/>`_ Dataset.
        Args:
            root (string): Root directory of the dataset.
            train (bool, optional): If True, creates dataset from training set, otherwise
    ...
    def __init__(self, root, train=True, transform=None, target_transform=None, download=False, k=4096):
        super(DogsSample, self).__init__(root, transform=transform, target_transform=target_transform)
        ...
        for i in range(num_samples):
            path, target = self._flat_breed_images[i]
            label[i] = target
